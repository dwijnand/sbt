  ...
  lazy val structure: BuildStructure = {
    ...
    val buildUnit: BuildUnit = {
      ...
      val project: Project = {
        ...
        resolveProject(project0, autoPlugins, loadedPlugins, injectSettings, fileToLoadedSbtFileMap,
          state.log)
      }
    ...
    val settings: Seq[Setting[_]] = finalTransforms(buildConfigurations(loadedBuild,
        getRootProject(units), config.injectSettings))
