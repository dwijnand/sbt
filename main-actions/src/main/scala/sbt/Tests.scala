...
object Tests {
  ...
  private[sbt] def processOptions(
    ...
    new ProcessedOptions(uniqueTests.toVector, setup.toVector, cleanup.toVector,
  ...
  def apply(
    ...
    testTask(testLoader, frameworks, runners, o.tests, o.setup, o.cleanup, log, o.testListeners,
      config)
  }
      ...
      case (nt, idx) =>
        ...
        (testFunDef.fullyQualifiedName,
          TestFramework.createTestFunction(loader,
            new TaskDef(testFunDef.fullyQualifiedName + "-" + idx, testFunDef.fingerprint,
              testFunDef.explicitlySpecified, testFunDef.selectors),
            testFun.runner, nt))
    }
