...
object GlobalPlugin {
  ...
  def inject(gp: GlobalPluginData): Seq[Setting[_]] = Seq[Setting[_]](
    projectDescriptors ~= (_ ++ gp.descriptors),
    projectDependencies ++= gp.projectID +: gp.dependencies,
    resolvers := (resolvers.value ++ gp.resolvers).distinct,
  ...
  private[this] def injectInternalClasspath(config: Configuration, cp: Seq[Attributed[File]]) =
  ...
  def build(base: File, s: State, config: LoadBuildConfiguration): (BuildStructure, State) = {
    ...
    val pluginManagement = config.pluginManagement.forGlobalPlugin
    val globalConfig = config.copy(injectSettings = newInject, pluginManagement = pluginManagement)
  ...
  def extract(state: State, structure: BuildStructure): (State, GlobalPluginData) = {
    ...
    val resolvedTaskInit = Project.inScope(p, taskInit)
  ...
  val globalPluginSettings = Project.inScope(Scope.GlobalScope in LocalRootProject)(
    Seq(
      ...
      onLoadMessage := Def.setting(s"Loading global plugins from ${baseDirectory.value}").value,
      name := "global-plugin",
      sbtPlugin := true,
      version := "0.0"
    )
