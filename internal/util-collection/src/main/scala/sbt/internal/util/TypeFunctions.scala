...
trait TypeFunctions {
  ...
  final val left: Id ~> AnyLeft = λ[Id ~> AnyLeft](Left(_)).setToString("TypeFunctions.left")
  final val right: Id ~> AnyRight = λ[Id ~> AnyRight](Right(_)).setToString("TypeFunctions.right")
...
trait ~>[-A[_], +B[_]] { self =>
  ...
  /** Converts this natural transformation into a monomorphic Function1 for given a type `T`. */
  final def fn[T]: A[T] => B[T] = (t: A[T]) => apply[T](t)

  /** Composition: `this ∙ g` means apply first `g` then `this`. */
  final def ∙[C[_]](g: C ~> A): C ~> B = λ[C ~> B](c => self.apply(g(c)))

  final def ∙[C, D](g: C => D)(implicit ev: D <:< A[D]): C => B[D] = c => apply(g(c))
}
