...
object Defaults extends BuildCommon {
  ...
  def configSrcSub(key: SettingKey[File]): Initialize[File] =
    Def.setting {
      (key in ThisScope.copy(config = Zero)).value / nameForSrc(
  ...
  private[sbt] lazy val globalJvmCore: Seq[Setting[_]] =
    Seq(
      ...
      fullJavaHomes := CrossJava.expandJavaHomes(
        discoveredJavaHomes.value ++ javaHomes.value
      ),
  ...
  private[sbt] lazy val globalIvyCore: Seq[Setting[_]] =
    Seq(
      ...
      sbtResolver := {
        ...
        if (v.endsWith("-SNAPSHOT") || v.contains("-bin-"))
          Classpaths.sbtMavenSnapshots
  ...
  private[sbt] lazy val globalSbtCore: Seq[Setting[_]] = globalDefaults(
    Seq(
      ...
      taskTemporaryDirectory := {
        val base = BuildPaths.globalTaskDirectoryStandard(
      ...
      progressState := {
        if ((ThisBuild / useSuperShell).value)
          Some(new ProgressState(SysProp.supershellBlankZone))
        else None
      },
      ...
      serverAuthentication := {
        if (serverConnectionType.value == ConnectionType.Tcp)
          Set(ServerAuthentication.Token)
        else Set()
      },
      serverHandlers :== Nil,
      fullServerHandlers := {
    ...
    unmanagedSources / fileInputs := {
      ...
      val baseSources =
        if (sourcesInBase.value)
          Globs(baseDirectory.value.toPath, recursive = false, filter) :: Nil
        else Nil
      unmanagedSourceDirectories.value
        .map(d => Globs(d.toPath, recursive = true, filter)) ++ baseSources
    },
    unmanagedSources := (unmanagedSources / inputFileStamps).value
      .map(_._1.toFile),
  ...
  lazy val resourceConfigPaths = Seq(
    ...
    unmanagedResources / fileInputs := {
      ...
      unmanagedResourceDirectories.value.map(
        d => Globs(d.toPath, recursive = true, filter)
      )
    },
    unmanagedResources := (unmanagedResources / inputFileStamps).value
      .map(_._1.toFile),
    resourceGenerators :== Nil,
    resourceGenerators += Def.task {
      PluginDiscovery
        .writeDescriptors(discoveredSbtPlugins.value, resourceManaged.value)
    },
  ...
  lazy val outputConfigPaths = Seq(
    ...
    semanticdbTargetRoot := crossTarget.value / (prefix(
      configuration.value.name
    ) + "meta"),
  ...
  def compileBase =
    inTask(console)(compilersSetting :: Nil) ++ compileBaseGlobal ++ Seq(
      ...
      scalaCompilerBridgeSource := ZincLmUtil.getDefaultBridgeModule(
        scalaVersion.value
      ),
      consoleProject / scalaCompilerBridgeSource := ZincLmUtil
        .getDefaultBridgeModule(
      ...
      if (java.lang.Boolean.getBoolean(
        ...
        compilers.withScalac(
          compilers.scalac match {
            case x: AnalyzingCompiler =>
              x.withClassLoaderCache(classLoaderCache)
            case x => x
          }
        )
      }
  ...
  lazy val configTasks: Seq[Setting[_]] = docTaskSettings(doc) ++ inTask(
    compile
  )(
    ...
    externalHooks := {
      ...
      ExternalHooks.default
        .value(inputChanges, outputChanges, fileTreeView.value)
    },
    compileSourceFileInputs := {
      ...
      val prev: Map[String, Seq[(java.nio.file.Path, sbt.nio.FileStamp)]] =
        compileSourceFileInputs.previous
          .map(_.filterKeys(versions))
          .getOrElse(Map.empty)
      prev + (version ->
        ((unmanagedSources / inputFileStamps).value ++ (managedSourcePaths / outputFileStamps).value))
    },
    ...
    compileBinaryFileInputs := {
      ...
      val versions = crossScalaVersions.value.toSet + version
      val prev: Map[String, Seq[(java.nio.file.Path, sbt.nio.FileStamp)]] =
        compileBinaryFileInputs.previous
          .map(_.filterKeys(versions))
          .getOrElse(Map.empty)
      prev + (version -> (dependencyClasspathFiles / outputFileStamps).value)
    },
    compileBinaryFileInputs := compileBinaryFileInputs
      .triggeredBy(compile)
      .value,
    incOptions := {
    ...
    selectMainClass := mainClass.value orElse askForMainClass(
      discoveredMainClasses.value
    ),
    mainClass in run := (selectMainClass in run).value,
    mainClass := pickMainClassOrWarn(
      discoveredMainClasses.value,
      streams.value.log
    ),
  ...
  lazy val projectTasks: Seq[Setting[_]] = Seq(
    ...
    clean := Def
      .taskDyn(Clean.task(resolvedScoped.value.scope, full = true))
      .value,
  ...
  def generate(
      generators: SettingKey[Seq[Task[Seq[File]]]]
  ...
  @deprecated(
    "This is no longer used to implement continuous execution",
    "1.3.0"
  )
  def watchSetting: Initialize[Watched] =
    Def.setting {
      ...
      val msg =
        watchingMessage.?.value.getOrElse(Watched.defaultWatchingMessage)
      val trigMsg =
        triggeredMessage.?.value.getOrElse(Watched.defaultTriggeredMessage)
      new Watched {
  ...
  def scalaInstanceFromUpdate: Initialize[Task[ScalaInstance]] = Def.task {
    val toolReport = update.value
      .configuration(Configurations.ScalaTool) getOrElse
  ...
  private[this] def mkScalaInstance(
    ...
    class ScalaLoader
        extends URLClassLoader(
          allJarsDistinct.map(_.toURI.toURL).toArray,
          libraryLoader
        )
  ...
  lazy val testTasks: Seq[Setting[_]] = testTaskOptions(test) ++ testTaskOptions(
    testOnly
  ) ++ testTaskOptions(
    testQuick
  ) ++ testDefaults ++ Seq(
    ...
    loadedTestFrameworks := {
      ...
      testFrameworks.value
        .flatMap(f => f.create(loader, log).map(x => (f, x)).toIterable)
        .toMap
    },
    ...
    test := {
      ...
      val taskName =
        Project.showContextKey(state.value).show(resolvedScoped.value)
      ...
      Seq(
        testListeners := {
          TestLogger.make(
            ...
            new TestStatusReporter(
              succeededFile(streams.in(test).value.cacheDirectory)
            ) +:
            testListeners.in(TaskZero).value
        },
        testOptions := Tests
          .Listeners(testListeners.value) +: (testOptions in TaskZero).value,
  ...
  private[this] def closeableTestLogger(
      manager: Streams,
      baseKey: Scoped,
      buffered: Boolean
  )(
  ...
  def singleTestGroupDefault: Initialize[Task[Seq[Tests.Group]]] = Def.task {
    ...
    val runPolicy = if (fork.value) Tests.SubProcess(opts) else Tests.InProcess
    Seq(Tests.Group("<default>", definedTests.value, runPolicy))
  }

  def forkOptionsTask: Initialize[Task[ForkOptions]] = Def.task {
    ForkOptions(
      javaHome = javaHome.value,
  ...
  def testExecutionTask(task: Scoped): Initialize[Task[Tests.Execution]] =
    inTask(
      task,
      Def.task(
        Tests.Execution(testOptions.value, parallelExecution.value, tags.value)
      )
    )
  ...
  def inputTests(key: InputKey[_]): Initialize[InputTask[Unit]] =
    inTask(key, inputTests0)

  private[this] lazy val inputTests0: Initialize[InputTask[Unit]] = {
    val parser = loadForParser(definedTestNames)(
      (s, i) => testOnlyParser(s, i getOrElse Nil)
    )
    Def.inputTaskDyn {
      ...
      val modifiedOpts = Tests.Filters(filter(selected)) +: Tests.Argument(
        frameworkOptions: _*
      ) +: config.options
    ...
    frameworks.map {
      case (tf, f) =>
        val args = opts.flatMap {
          case Tests.Argument(None | Some(`tf`), args) => args
          case _                                       => Nil
        }
  ...
  private[sbt] def allTestGroupsTask(
    ...
    val strategy = ClassLoaderLayeringStrategy.ScalaLibrary
    allTestGroupsTask(
      s,
      frameworks,
      loader,
  ...
  private[sbt] def allTestGroupsTask(
    ...
    val forked = forkedParallelExecution
    val strategy = ClassLoaderLayeringStrategy.ScalaLibrary
    allTestGroupsTask(
      s,
      frameworks,
  ...
  def selectedFilter(args: Seq[String]): Seq[String => Boolean] = {
    ...
    (includeFilters, excludeArgs) match {
      ...
      case _ =>
        includeFilters.map(
          f => (s: String) => (f.accept(s) && !matches(excludeFilters, s))
        )
    }
  }

  def detectTests: Initialize[Task[Seq[TestDefinition]]] = Def.task {
    Tests
      .discover(
        loadedTestFrameworks.value.values.toList,
        compile.value,
        streams.value.log
      )
      ._1
  ...
  lazy val packageBase: Seq[Setting[_]] = Def.settings(
  ...
  lazy val packageConfig: Seq[Setting[_]] = Def.settings(
    inTask(packageBin)(
      Seq(
        packageOptions := {
          val n = name.value
          ...
          Package.addSpecManifestAttributes(n, ver, orgName) +:
            Package.addImplManifestAttributes(
              n,
              ver,
              homepage.value,
              org,
    ...
    inTask(packageSrc)(
      Seq(
        packageOptions := {
          ...
          packageOption +: packageOptions.value
        }
      )
    ),
  ...
  def collectFiles(
    ...
    Def.task {
      dirs.toTask.value
        .descendantsExcept(filter.toTask.value, excludes.toTask.value)
        .get
    }
  ...
  def artifactPathSetting(art: SettingKey[Artifact]): Initialize[File] =
    Def.setting {
      ...
      crossTarget.value / artifactName.value(scalaV, projectID.value, art.value)
    }
  ...
  def artifactSetting: Initialize[Artifact] =
    Def.setting {
      ...
      if (combined.isEmpty)
        a.withClassifier(None).withConfigurations(configurations)
      else {
  ...
  @deprecated(
    "The configuration(s) should not be decided based on the classifier.",
    "1.0.0"
  )
  def artifactConfigurations(
    ...
    classifier.fold(scope :: Nil)(Artifact.classifierConf(_) :: Nil)
  ...
  def packageTask: Initialize[Task[File]] =
    Def.task {
      ...
      Package(config, s.cacheStoreFactory, s.log, time)
      config.jar
    }
      ...
      service.runInBackgroundWithLoader(resolvedScoped.value, state.value) { (logger, workingDir) =>
        ...
        scalaRun.value match {
          case r: Run =>
            ...
            (
              Some(loader),
              () => r.runWithLoader(loader, cp, mainClass, args, logger).get
            )
          case sr =>
  ...
  def bgRunTask(
    ...
    Def.inputTask {
      val service = bgJobService.value
      val mainClass = mainClassTask.value getOrElse sys.error(
  ...
  def runMainTask(
    ...
    val parser =
      loadForParser(discoveredMainClasses)(
        (s, names) => runMainParser(s, names getOrElse Nil)
      )
    Def.inputTask {
      val (mainClass, args) = parser.parsed
      scalaRun.value
        .run(mainClass, data(classpath.value), args, streams.value.log)
        .get
    }
  }
  ...
  def bgStopTask: Initialize[InputTask[Unit]] =
    foreachJobTask((manager, handle) => manager.stop(handle))

  def bgWaitForTask: Initialize[InputTask[Unit]] =
    foreachJobTask { (manager, handle) =>
      ...
      Seq(
        apiMappings ++= {
          ...
          if (autoAPIMappings.value)
            APIMappings.extract(dependencyCp, log).toMap
        ...
        key in TaskZero := {
          ...
          (hasScala, hasJava) match {
            ...
            case (_, true) =>
              val javadoc =
                sbt.inc.Doc.cachedJavadoc(
                  label,
                  s.cacheStoreFactory sub "java",
                  cs.javaTools
                )
              javadoc.run(
                srcs.toList,
  ...
  def consoleProjectTask =
    Def.task {
      ConsoleProject(state.value, (initialCommands in consoleProject).value)(
        streams.value.log
      )
      println()
    }
    ...
    Def.task {
      ...
      val loader = sbt.internal.inc.classpath.ClasspathUtilities
        .makeLoader(
          fullcp,
          si,
          IO.createUniqueDirectory((taskTemporaryDirectory in task).value)
        )
      val compiler =
  ...
  def compileTask: Initialize[Task[CompileAnalysis]] = Def.task {
    ...
    if (analysisResult.hasModified) {
      ...
      val contents = AnalysisContents
        .create(analysisResult.analysis(), analysisResult.setup())
      store.set(contents)
    }
    ...
    analysis.readStamps.getAllProductStamps.asScala.foreach {
      case (f, s) =>
        map.put(
          f.toPath,
          sbt.nio.FileStamp.LastModified(s.getLastModified.orElse(-1L))
        )
    }
    analysis
  }
  ...
  def printWarningsTask: Initialize[Task[Unit]] =
    Def.task {
      ...
      val reporter =
        new ManagedLoggedReporter(max, streams.value.log, foldMappers(spms))
  ...
  def discoverSbtPluginNames: Initialize[Task[PluginDiscovery.DiscoveredNames]] = Def.taskDyn {
    if (sbtPlugin.value)
      Def.task(PluginDiscovery.discoverSourceAll(compile.value))
  ...
  private def distinctParser(
    ...
    import DefaultParsers._
    import Parser.and
    val base = token(Space) ~> token(
      and(NotSpace, not("--", "Unexpected: ---")) examples exs
    )
  ...
  lazy val disableAggregation =
    Defaults.globalDefaults(noAggregation map disableAggregate)
...
object Classpaths {
  ...
  private[this] def classpaths: Seq[Setting[_]] =
    Seq(
      externalDependencyClasspath := concat(
        unmanagedClasspath,
        managedClasspath
      ).value,
      dependencyClasspath := concat(
      ...
      exportedProductsIfMissing := trackedExportedProducts(
        TrackLevel.TrackIfMissing
      ).value,
      exportedProductsNoTracking := trackedExportedProducts(
        TrackLevel.NoTracking
      ).value,
      exportedProductJars := trackedExportedJarProducts(TrackLevel.TrackAlways).value,
      exportedProductJarsIfMissing := trackedExportedJarProducts(
        TrackLevel.TrackIfMissing
      ).value,
      exportedProductJarsNoTracking := trackedExportedJarProducts(
        TrackLevel.NoTracking
      ).value,
      ...
      fullClasspathAsJars := concatDistinct(
        exportedProductJars,
      ...
      dependencyClasspathFiles / outputFileStamps := {
        ...
        dependencyClasspathFiles.value.flatMap(
          p => cache.getOrElseUpdate(p, stamper).map(p -> _)
        )
      }
    )
  ...
  private[this] def exportClasspath(
    ...
    s.mapInitialize(
      init => Def.task { exportClasspath(streams.value, init.value) }
    )
  private[this] def exportClasspath(
      s: TaskStreams,
      cp: Classpath
  ...
  def findClasspathConfig(
    ...
    val search = map(thisConfig) +: (delegated.toList ++ Seq(
      Compile,
  ...
  ): Initialize[Task[Map[Artifact, File]]] =
    enabledOnly(packagedArtifact.toSettingKey, pkgTasks) apply (_.join
      .map(_.toMap))
    ...
    Defaults.globalDefaults(
      Seq(
        ...
        cleanKeepGlobs := {
          ...
          val dirs = BuildPaths
            .globalLoggingStandard(base) :: BuildPaths
            .globalTaskDirectoryStandard(base) :: Nil
        ...
        sbtDependency := {
          ...
          CrossVersion(scalaVersion, binVersion)(base)
            .withCrossVersion(Disabled())
        },
  ...
  val ivyBaseSettings: Seq[Setting[_]] = baseGlobalDefaults ++ sbtClassifiersTasks ++ Seq(
    conflictWarning := conflictWarning.value
      .copy(label = Reference.display(thisProjectRef.value)),
    ...
    externalResolvers := ((
      ...
      case (_, rs, _, uj) =>
        Resolver.combineDefaultResolvers(rs.toVector, uj, mavenCentral = true)
    }),
    appResolvers := {
    ...
    fullResolvers :=
      (Def.task {
        ...
        bootResolvers.value match {
          ...
          case _ =>
            val base =
              if (sbtPlugin.value) sbtResolvers.value ++ rs ++ pr else rs ++ pr
            (proj +: base).distinct
        }
      }).value,
      ...
      if (retrieveManaged.value)
        Some(
          RetrieveConfiguration()
            ...
            .withConfigurationsToRetrieve(configurationsToRetrieve.value map {
              _.toVector
            })
        )
      else None
    ...
    inTask(updateClassifiers)(
      Seq(
        classifiersModule := {
          implicit val key =
            (m: ModuleID) => (m.organization, m.name, m.revision)
          val projectDeps = projectDependencies.value.iterator.map(key).toSet
          val externalModules =
            update.value.allModules.filterNot(m => projectDeps contains key(m))
    ...
    LMCoursier.publicationsSetting(
      Seq(Compile, Test).map(c => c -> CConfiguration(c.name))
    )
  ...
  val jvmBaseSettings: Seq[Setting[_]] = Seq(
    ...
    allDependencies := {
      ...
      val sbtdeps =
        (sbtDependency in pluginCrossBuild).value
          .withConfigurations(Some(Provided.name))
      val pluginAdjust =
      ...
      if (scalaHome.value.isDefined || scalaModuleInfo.value.isEmpty || !managedScalaInstance.value)
        ...
        ScalaArtifacts
          .toolDependencies(sbtOrg, version, isDotty) ++ pluginAdjust
      }
    },
    ...
    dependencyOverrides ++= {
      ...
      val id = app.provider.id
      val sv = (sbtVersion in pluginCrossBuild).value
      val base = ModuleID(id.groupID, "scripted-plugin", sv)
  ...
  private[sbt] def warnInsecureProtocol(
      ress: Seq[Resolver],
      log: Logger
  ): Unit = {
  ...
  private[sbt] def warnInsecureProtocolInModules(
      mods: Seq[ModuleID],
      log: Logger
  ): Unit = {
        ...
        Seq(
          ...
          updateSbtClassifiers in TaskGlobal := (Def.task {
            ...
            val updateConfig = updateConfig0
              ...
              .withArtifactFilter(
                updateConfig0.artifactFilter
                  .map(af => af.withInverted(!af.inverted))
              )
            val app = appConfiguration.value
  ...
  def classifiersModuleTask: Initialize[Task[GetClassifiersModule]] =
    Def.task {
      ...
      val pluginClasspath =
        loadedBuild.value.units(ref.build).unit.plugins.fullClasspath.toVector
  ...
  val moduleIdJsonKeyFormat: sjsonnew.JsonKeyFormat[ModuleID] =
    new sjsonnew.JsonKeyFormat[ModuleID] {
      ...
      val moduleIdFormat: JsonFormat[ModuleID] =
        implicitly[JsonFormat[ModuleID]]
      def write(key: ModuleID): String =
        CompactPrinter(Converter.toJsonUnsafe(key)(moduleIdFormat))
      def read(key: String): ModuleID =
        Converter.fromJsonUnsafe[ModuleID](Parser.parseUnsafe(key))(
          moduleIdFormat
        )
    }
   ...
   */
  private def unmanagedJarsTask(
      scalaVersion: String,
      unmanagedVersion: String,
      jars: Seq[File]
  ) = { (subVersion0: String) =>
  ...
  def updateTask: Initialize[Task[UpdateReport]] =
    updateTask0("updateFull", true, true)
  ...
  private def updateTask0(
    ...
    val cacheStoreFactory: CacheStoreFactory = {
      val factory =
        state.value
          .get(Keys.cacheStoreFactoryFactory)
          .getOrElse(InMemoryCacheStore.factory(0))
    ...
    val updateConf = {
      ...
      val conf1 = maybeUpdateLevel.orElse(state0.get(logLevel.key)) match {
        case Some(Level.Debug) if conf.logging == Default =>
          conf.withLogging(logging = Full)
        case Some(_) if conf.logging == Default =>
          conf.withLogging(logging = DownloadOnly)
        case _ => conf
      }
    ...
    Def.task {
      ...
      val f = Tracked.inputChanged(cacheStoreFactory make "input_dsp") {
        (inChanged: Boolean, in: Seq[ModuleID]) =>
          implicit val NoPositionFormat: JsonFormat[NoPosition.type] =
            asSingleton(NoPosition)
          ...
          implicit val RangePositionFormat
              : IsoLList.Aux[RangePosition, String :*: LineRange :*: LNil] =
            LList.iso(
              { r: RangePosition =>
  ...
  def defaultRepositoryFilter: MavenRepository => Boolean =
    repo => !repo.root.startsWith("file:")
    ...
    Def.task {
      ...
      deps.classpath(ref) flatMap { dep =>
        (projectID in dep.project) get data map {
          _.withConfigurations(dep.configuration)
  ...
  private[sbt] def trackedExportedProducts(
      track: TrackLevel
  ...
  private[sbt] def trackedExportedJarProducts(
      track: TrackLevel
  ): Initialize[Task[Classpath]] =
    Def.task {
      val _ = (packageBin / dynamicDependency).value
    ...
    Def.taskDyn {
      ...
      TrackLevel.intersection(track, exportToInternal.value) match {
        ...
        case _ =>
          Def.task {
            val analysis =
              previousCompile.value.analysis.toOption.getOrElse(Analysis.empty)
            dirs.map(_ -> analysis)
          }
      }
    }
  ...
  def mkIvyConfiguration: Initialize[Task[InlineIvyConfiguration]] =
    Def.task {
      val (rs, other) =
        (fullResolvers.value.toVector, otherResolvers.value.toVector)
      val s = streams.value
  ...
  def parseMapping(
    ...
    union(
      confString
        .split(";") map parseSingleMapping(masterConfs, depConfs, default)
    )
  ...
  private[sbt] def productsTask(
    ...
    track match {
      case TrackLevel.NoTracking =>
        getClasspath(exportedProductsNoTracking, dep, conf, data)
      case TrackLevel.TrackIfMissing =>
        getClasspath(exportedProductsIfMissing, dep, conf, data)
      case TrackLevel.TrackAlways =>
  ...
  private[sbt] def jarProductsTask(
    ...
    track match {
      case TrackLevel.NoTracking =>
        getClasspath(exportedProductJarsNoTracking, dep, conf, data)
      case TrackLevel.TrackIfMissing =>
        getClasspath(exportedProductJarsIfMissing, dep, conf, data)
      case TrackLevel.TrackAlways =>
        getClasspath(exportedProductJars, dep, conf, data)
    }
  ...
  val sbtMavenSnapshots: MavenRepository =
    MavenRepository(
      "sbt-maven-snapshot",
      Resolver.SbtRepositoryRoot + "/" + "maven-snapshots/"
    )
  ...
  def autoPlugins(
    ...
    val pluginClasspath = report.matching(
      configurationFilter(CompilerPlugin.name)
    ) ++ internalPluginClasspath
    val plugins =
      sbt.internal.inc.classpath.ClasspathUtilities
        .compilerPlugins(pluginClasspath, isDotty)
  ...
  private[this] lazy val internalCompilerPluginClasspath: Initialize[Task[Classpath]] =
    Def.taskDyn {
      ...
      val data = settingsData.value
      val deps = buildDependencies.value
      val track = TrackLevel.TrackAlways
      internalDependenciesImplTask(ref, CompilerPlugin, CompilerPlugin, data, deps, track)
    }
  ...
  def substituteScalaFiles(scalaOrg: String, report: UpdateReport)(
    ...
    report.substitute { (_, module, arts) =>
  ...
  def bootChecksums(app: xsbti.AppConfiguration): Vector[String] =
    try app.provider.scalaProvider.launcher.checksums.toVector
  ...
  def appRepositories(app: xsbti.AppConfiguration): Option[Vector[Resolver]] =
    try Some(app.provider.scalaProvider.launcher.appRepositories.toVector.map(bootRepository))
    catch { case _: NoSuchMethodError => None }

  def bootRepositories(app: xsbti.AppConfiguration): Option[Vector[Resolver]] =
    try Some(app.provider.scalaProvider.launcher.ivyRepositories.toVector.map(bootRepository))
    catch { case _: NoSuchMethodError => None }
  ...
  @silent private[this] def bootRepository(repo: xsbti.Repository): Resolver = {
  ...
  def shellPromptFromState: State => String = { s: State =>
    ...
    (name in extracted.currentRef).get(extracted.structure.data) match {
      case Some(name) =>
        s"sbt:$name" + Def.withColor("> ", Option(scala.Console.CYAN))
      case _ => "> "
    }
  }
}
...
trait BuildExtra extends BuildCommon with DefExtra {
  ...
  def externalIvySettingsURI(
    ...
    ivyConfiguration := uri
      .zipWith(otherArgs) { (u, otherTask) =>
        otherTask.map {
  ...
  def fullRunInputTask(
    ...
    def inputTask[T](f: TaskKey[Seq[String]] => Initialize[Task[T]]) =
      InputTask(Def.value((_: State) => Def.spaceDelimited()))(f)

    Vector(
      scoped := inputTask { result =>
        val runner = initScoped(scoped.scopedKey, inConfig(config, ClassLoaders.runner))
        val args = Def.task(((fullClasspath in config).value, streams.value, result.value))
        runner.zipWith(args) { (runner, args) =>
          (runner, args).map {
            case (r, (cp, s, args)) => r.run(mainClass, data(cp), baseArguments ++ args, s.log).get
          }
        }
      }.evaluated,
      (forkOptions in config := forkOptionsTask.value)
        .transform(Def.mapScope(Scope.replaceThis(ThisScope in scoped.key))),
    )
  }
  ...
  def fullRunTask(
    ...
    Vector(
      scoped := {
        initScoped(scoped.scopedKey, inConfig(config, ClassLoaders.runner))
          .zipWith(Def.task(((fullClasspath in config).value, streams.value)))(
            (runner, tuple) =>
              (runner, tuple).map {
                case (r, (cp, s)) => r.run(mainClass, data(cp), arguments, s.log).get
              }
          )
          .value
      },
  ...
  def initScope[T](s: Scope, i: Initialize[T]): Initialize[T] = Project.inScope(s, i)
...
trait BuildCommon {
  ...
  def loadFromContext[T: JsonFormat](task: TaskKey[T], context: ScopedKey[_], s: State): Option[T] =
  ...
  def loadForParserI[P, T: JsonFormat](task: TaskKey[T])(
      init: Initialize[(State, Option[T]) => Parser[P]]
  ): Initialize[State => Parser[P]] =
    Def.setting((s: State) => init.value(s, loadFromContext(task, resolvedScoped.value, s)))

  def getForParser[P, T](task: TaskKey[T])(
      init: (State, Option[T]) => Parser[P],
  ): Initialize[State => Parser[P]] =
  ...
  def loadPrevious[T: JsonFormat](task: TaskKey[T]): Initialize[Task[Option[T]]] =
    Def.task(loadFromContext(task, resolvedScoped.value, state.value))
